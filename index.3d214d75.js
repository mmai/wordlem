...
*/ var _Scheduler_working = false;
    ...
    var $author$project$Main$initialModel = F4(function(store, key, wordRef, state) {
        return {
            key: key,
    ...
    var $author$project$Main$WordsReceivedForWordRef = function(a) {
        return {
            $: 'WordsReceivedForWordRef',
            a: a
        };
    };
    ...
    var $author$project$Main$startNewGame = F2(function(_v0, maybeWordRef) {
        var lang = _v0.lang;
        var settings = _v0.settings;
        var _v1 = _Utils_Tuple2(maybeWordRef, settings.wordSize);
        _v1$0: while(true)if (_v1.b.$ === 'Just') {
            if (_v1.a.$ === 'Just') break _v1$0;
            else return A2($author$project$Client$getWords, lang, $author$project$Main$WordsReceived);
        } else {
            if (_v1.a.$ === 'Just') break _v1$0;
            else {
                var _v2 = _v1.b;
                return $author$project$Main$getRandomWordSize;
            }
        }
        var wordRef = _v1.a.a;
        return A2($author$project$Client$getWords, $author$project$I18n$parseLang(wordRef.lang), $author$project$Main$WordsReceivedForWordRef);
    });
    ...
    var $author$project$Main$WordUri = F3(function(a, b, c) {
        return {
            $: 'WordUri',
            a: a,
            b: b,
    ...
    var $author$project$Main$routeParser = $elm$url$Url$Parser$oneOf(_List_fromArray([
        A2($elm$url$Url$Parser$map, $author$project$Main$WordUri, A2($elm$url$Url$Parser$slash, $elm$url$Url$Parser$string, A2($elm$url$Url$Parser$slash, $elm$url$Url$Parser$int, $elm$url$Url$Parser$string)))
    ]));
    var $author$project$Main$urlFragmentToPath = function(url) {
        return _Utils_update(url, {
            fragment: $elm$core$Maybe$Nothing,
            path: A2($elm$core$Maybe$withDefault, '', url.fragment)
        });
    };
    var $author$project$Main$urlToWordref = function(url) {
        return A2($elm$core$Maybe$map, function(_v0) {
            var lang = _v0.a;
            var wordSize = _v0.b;
            var wordId = _v0.c;
            return {
        ...
        }, A2($elm$url$Url$Parser$parse, $author$project$Main$routeParser, $author$project$Main$urlFragmentToPath(url)));
    };
    var $author$project$Main$init = F3(function(flags, url, key) {
        var maybeWordRef = $author$project$Main$urlToWordref(url);
        var defaultLang = flags.lang;
        var _v0 = function() {
            var _v1 = $author$project$Store$fromJson(flags.rawStore);
            if (_v1.$ === 'Ok') {
                var store = _v1.a;
                var defaultSize = A2($elm$core$Maybe$withDefault, 5, store.settings.wordSize);
                ...
                var store = $author$project$Store$default($author$project$I18n$parseLang(wordRef.lang));
                var newModel = A4($author$project$Main$initialModel, store, key, wordRef, $author$project$Game$Idle);
        ...
        return _Utils_Tuple2(model, $elm$core$Platform$Cmd$batch(_List_fromArray([
            A2($author$project$Main$startNewGame, model.store, maybeWordRef),
            cmds
        ])));
    });
    ...
    var $pdamoc$elm_hashids$Hashids$ordOfIdx = F2(function(i, str) {
        return A2($elm$core$Maybe$withDefault, 0, A2($elm$core$Maybe$map, $elm$core$Char$toCode, $elm$core$List$head($elm$core$String$toList(A2($elm$core$String$dropLeft, i, str)))));
    });
    var $pdamoc$elm_hashids$Hashids$forceGet = F2(function(i, axs) {
        return A2($elm$core$Maybe$withDefault, _Utils_chr(' '), A2($elm$core$Array$get, i, axs));
    });
    ...
    var $pdamoc$elm_hashids$Hashids$swap = F3(function(i, j, str) {
        var strArray = $elm$core$Array$fromList($elm$core$String$toList(str));
        var jElem = A2($pdamoc$elm_hashids$Hashids$forceGet, j, strArray);
        var iElem = A2($pdamoc$elm_hashids$Hashids$forceGet, i, strArray);
        return $elm$core$String$fromList($elm$core$Array$toList(A3($elm$core$Array$set, j, iElem, A3($elm$core$Array$set, i, jElem, strArray))));
    });
    var $pdamoc$elm_hashids$Hashids$reorder = F2(function(string, salt) {
        var saltLen = $elm$core$String$length(salt);
        var saltArray = $elm$core$Array$fromList($elm$core$String$toList(salt));
        var shuffle = F4(function(i, index, integerSum, str) {
            shuffle: while(true){
                if (i > 0) {
                    var newIndex = A2($elm$core$Basics$modBy, saltLen, index);
                    var integer = $elm$core$Char$toCode(A2($pdamoc$elm_hashids$Hashids$forceGet, newIndex, saltArray));
                    var newIntegerSum = integerSum + integer;
                    var j = A2($elm$core$Basics$modBy, i, integer + newIndex + newIntegerSum);
                    var newStr = A3($pdamoc$elm_hashids$Hashids$swap, i, j, str);
                    var $temp$i = i - 1, $temp$index = newIndex + 1, $temp$integerSum = newIntegerSum, $temp$str = newStr;
                    i = $temp$i;
                    index = $temp$index;
                    integerSum = $temp$integerSum;
                    str = $temp$str;
                    continue shuffle;
                } else return str;
            }
        });
        var alphaLen = $elm$core$String$length(string);
        return !saltLen ? string : A4(shuffle, alphaLen - 1, 0, 0, string);
    });
    var $pdamoc$elm_hashids$Hashids$strGet = F2(function(i, str) {
        return A2($elm$core$String$left, 1, A2($elm$core$String$dropLeft, i, str));
    });
    var $pdamoc$elm_hashids$Hashids$ensureLength = F5(function(encoded, minHashLength, alphabet, guards, valuesHash) {
        var splitAt = $elm$core$String$length(alphabet) / 2 | 0;
        var guardsLen = $elm$core$String$length(guards);
        var guardIndex = A2($elm$core$Basics$modBy, guardsLen, valuesHash + A2($pdamoc$elm_hashids$Hashids$ordOfIdx, 0, encoded));
        var extend = F2(function(encoded_, alpha) {
            extend: while(true){
                var alpha1 = A2($pdamoc$elm_hashids$Hashids$reorder, alpha, alpha);
                var encodedPre = _Utils_ap(A2($elm$core$String$dropLeft, splitAt, alpha1), _Utils_ap(encoded_, A2($elm$core$String$left, splitAt, alpha1)));
                var excess = $elm$core$String$length(encodedPre) - minHashLength;
                var fromIndex = excess / 2 | 0;
                var encoded1 = excess > 0 ? A2($elm$core$String$left, minHashLength, A2($elm$core$String$dropLeft, fromIndex, encodedPre)) : encodedPre;
                if (_Utils_cmp($elm$core$String$length(encoded1), minHashLength) < 0) {
                    var $temp$encoded_ = encoded1, $temp$alpha = alpha1;
                    encoded_ = $temp$encoded_;
                    alpha = $temp$alpha;
                    continue extend;
                } else return encoded1;
            }
        });
        var acc = _Utils_ap(A2($pdamoc$elm_hashids$Hashids$strGet, guardIndex, guards), encoded);
        var guardIndex1 = A2($elm$core$Basics$modBy, guardsLen, valuesHash + A2($pdamoc$elm_hashids$Hashids$ordOfIdx, 2, acc));
        var acc1 = _Utils_cmp($elm$core$String$length(acc), minHashLength) < 0 ? _Utils_ap(acc, A2($pdamoc$elm_hashids$Hashids$strGet, guardIndex1, guards)) : acc;
        return A2(extend, acc1, alphabet);
    });
    var $elm$core$List$sum = function(numbers) {
        return A3($elm$core$List$foldl, $elm$core$Basics$add, 0, numbers);
    };
    var $pdamoc$elm_hashids$Hashids$encodeList = F2(function(context, numbers) {
        var valuesHash = $elm$core$List$sum(A2($elm$core$List$indexedMap, F2(function(i, n) {
            return A2($elm$core$Basics$modBy, i + 100, n);
        }), numbers));
        var _v0 = context;
        var guards = _v0.guards;
        var seps = _v0.seps;
        var salt = _v0.salt;
        var minHashLength = _v0.minHashLength;
        var alphabet = _v0.alphabet;
        var alphaLen = $elm$core$String$length(alphabet);
        var hash = F3(function(value, alpha, acc) {
            hash: while(true){
                var value1 = value / alphaLen | 0;
                var alphaLen_ = $elm$core$String$length(alpha);
                var acc1 = _Utils_ap(A2($pdamoc$elm_hashids$Hashids$strGet, A2($elm$core$Basics$modBy, alphaLen_, value), alpha), acc);
                if (!value1) return acc1;
                else {
                    var $temp$value = value1, $temp$alpha = alpha, $temp$acc = acc1;
                    value = $temp$value;
                    alpha = $temp$alpha;
                    acc = $temp$acc;
                    continue hash;
        ...
        var lottery = A2($pdamoc$elm_hashids$Hashids$strGet, A2($elm$core$Basics$modBy, alphaLen, valuesHash), alphabet);
        var sepsLen = $elm$core$String$length(seps);
        var go = F2(function(_v2, _v3) {
            var i = _v2.a;
            var value = _v2.b;
            var acc = _v3.a;
            var alpha = _v3.b;
            var alphaSalt = A2($elm$core$String$left, alphaLen, _Utils_ap(lottery, _Utils_ap(salt, alpha)));
            var alpha1_ = A2($pdamoc$elm_hashids$Hashids$reorder, alpha, alphaSalt);
            var last = A3(hash, value, alpha1_, '');
            var firstCode = A2($pdamoc$elm_hashids$Hashids$ordOfIdx, 0, last);
            var value1 = A2($elm$core$Basics$modBy, firstCode + i, value);
            var sepEnc = A2($pdamoc$elm_hashids$Hashids$strGet, A2($elm$core$Basics$modBy, sepsLen, value1), seps);
            return _Utils_Tuple2(_Utils_ap(acc, _List_fromArray([
                _Utils_ap(last, sepEnc)
            ])), alpha1_);
        });
        var _v1 = A3($elm$core$List$foldl, go, _Utils_Tuple2(_List_fromArray([
            lottery
        ]), alphabet), A2($elm$core$List$indexedMap, F2(function(a, b) {
            return _Utils_Tuple2(a, b);
        }), numbers));
        var encodedList = _v1.a;
        var alpha1 = _v1.b;
        var encodedPre = $elm$core$String$concat(encodedList);
        var encoded = A2($elm$core$String$dropRight, 1, encodedPre);
        return _Utils_cmp($elm$core$String$length(encoded), minHashLength) > -1 ? encoded : A5($pdamoc$elm_hashids$Hashids$ensureLength, encoded, minHashLength, alpha1, guards, valuesHash);
    });
    var $pdamoc$elm_hashids$Hashids$encode = F2(function(context, n) {
        return A2($pdamoc$elm_hashids$Hashids$encodeList, context, _List_fromArray([
            n
        ]));
    });
    ...
    var $pdamoc$elm_hashids$Hashids$defaultAlphabet = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890';
    var $pdamoc$elm_hashids$Hashids$containsChar = A2($elm$core$Basics$composeL, $elm$core$String$contains, $elm$core$String$fromChar);
    var $elm$core$String$filter = _String_filter;
    var $pdamoc$elm_hashids$Hashids$exclude = F2(function(toBeExcluded, from) {
        var member = function(c) {
            return !A2($pdamoc$elm_hashids$Hashids$containsChar, c, toBeExcluded);
        };
        return A2($elm$core$String$filter, member, from);
    });
    var $pdamoc$elm_hashids$Hashids$intersect = F2(function(first, second) {
        var member = function(c) {
            return A2($pdamoc$elm_hashids$Hashids$containsChar, c, second);
        };
        return A2($elm$core$String$filter, member, first);
    });
    var $pdamoc$elm_hashids$Hashids$unique = function(str) {
        var addIfNotMember = F2(function(c, xs) {
            return A2($pdamoc$elm_hashids$Hashids$containsChar, c, xs) ? xs : A2($elm$core$String$cons, c, xs);
        });
        return A3($elm$core$String$foldr, addIfNotMember, '', str);
    };
    var $pdamoc$elm_hashids$Hashids$createHashidsContext = F3(function(salt, minHashLen, alphabet) {
        var sepDiv = 3.5;
        var validSeps = function(_v3) {
            var seps = _v3.a;
            var alpha = _v3.b;
            var seps1 = A2($pdamoc$elm_hashids$Hashids$reorder, seps, salt);
            var lenSeps = $elm$core$String$length(seps1);
            var lenAlpha = $elm$core$String$length(alpha);
            var minSeps = $elm$core$Basics$ceiling(lenAlpha / sepDiv);
            if (_Utils_cmp(lenSeps, minSeps) < 0) {
                var minSeps1 = minSeps === 1 ? 2 : minSeps;
                if (_Utils_cmp(minSeps1, lenSeps) > 0) {
                    var splitAt = minSeps1 - lenSeps;
                    var seps2 = _Utils_ap(seps1, A2($elm$core$String$left, splitAt, alpha));
                    var alpha1 = A2($elm$core$String$dropLeft, splitAt, alpha);
                    return _Utils_Tuple2(seps2, alpha1);
                } else return _Utils_Tuple2(seps1, alpha);
            } else return _Utils_Tuple2(seps1, alpha);
        };
        var minAlphabetLength = 16;
        var guardDiv = 12;
        var withGuard = function(_v2) {
            var seps = _v2.a;
            var alpha = _v2.b;
            var lenAlpha = $elm$core$String$length(alpha);
            var numGuards = $elm$core$Basics$ceiling(lenAlpha / guardDiv);
            var alpha1 = A2($pdamoc$elm_hashids$Hashids$reorder, alpha, salt);
            return lenAlpha < 3 ? _Utils_Tuple3(A2($elm$core$String$dropLeft, numGuards, seps), alpha1, A2($elm$core$String$left, numGuards, seps)) : _Utils_Tuple3(seps, A2($elm$core$String$dropLeft, numGuards, alpha1), A2($elm$core$String$left, numGuards, alpha1));
        };
        var clean = function(alpha) {
            var seps = 'cfhistuCFHISTU';
            var seps1 = A2($pdamoc$elm_hashids$Hashids$intersect, seps, alpha);
            var hasSpaces = A2($elm$core$String$contains, ' ', alpha);
            var alpha1 = A2($pdamoc$elm_hashids$Hashids$exclude, seps1, $pdamoc$elm_hashids$Hashids$unique(alpha));
            var alphabetIsSmall = _Utils_cmp($elm$core$String$length(_Utils_ap(alpha1, seps1)), minAlphabetLength) < 0;
            var _v1 = _Utils_Tuple2(hasSpaces, alphabetIsSmall);
            if (_v1.a) return _Utils_Tuple2(seps, A2($pdamoc$elm_hashids$Hashids$exclude, seps, $pdamoc$elm_hashids$Hashids$defaultAlphabet));
            else {
                if (_v1.b) return _Utils_Tuple2(seps, A2($pdamoc$elm_hashids$Hashids$exclude, seps, $pdamoc$elm_hashids$Hashids$defaultAlphabet));
                else return _Utils_Tuple2(seps1, alpha1);
            }
        };
        var _v0 = withGuard(validSeps(clean(alphabet)));
        var seps1b = _v0.a;
        var alphabet1 = _v0.b;
        var guards = _v0.c;
        return {
    ...
    var $pdamoc$elm_hashids$Hashids$hashidsSimple = function(salt) {
        return A3($pdamoc$elm_hashids$Hashids$createHashidsContext, salt, 0, $pdamoc$elm_hashids$Hashids$defaultAlphabet);
    };
    var $author$project$Main$hashids = $pdamoc$elm_hashids$Hashids$hashidsSimple('wordelm salt');
    var $author$project$Main$pickWord = F2(function(words, _int) {
        return _Utils_Tuple2(A2($pdamoc$elm_hashids$Hashids$encode, $author$project$Main$hashids, _int), A2($elm_community$list_extra$List$Extra$getAt, _int, words));
    });
    var $author$project$Main$randomWord = function(words) {
        return A2($elm$random$Random$andThen, function(_int) {
            return $elm$random$Random$constant(A2($author$project$Main$pickWord, words, _int));
    ...
    var $pdamoc$elm_hashids$Hashids$regex = function(str) {
        return A2($elm$core$Maybe$withDefault, $elm$regex$Regex$never, $elm$regex$Regex$fromString(str));
    };
    var $elm$regex$Regex$split = _Regex_splitAtMost(_Regex_infinity);
    var $pdamoc$elm_hashids$Hashids$splitOn = F2(function(splitters, str) {
        return A2($elm$regex$Regex$split, $pdamoc$elm_hashids$Hashids$regex('[' + (splitters + ']')), str);
    });
    var $pdamoc$elm_hashids$Hashids$decode = F2(function(context, hash) {
        if (hash === '') return _List_Nil;
        else {
            var unhash = F2(function(part, alpha) {
                var position = function(c) {
                    return A3($elm$core$Basics$composeR, $elm$core$List$head, $elm$core$Maybe$withDefault(0), A2($elm$core$String$indexes, c, alpha));
                };
                var partList = A2($elm$core$List$map, $elm$core$String$fromChar, $elm$core$String$toList(part));
                var partLen = $elm$core$String$length(part);
                var alphaLen = $elm$core$String$length(alpha);
                return $elm$core$List$sum(A2($elm$core$List$indexedMap, F2(function(i, c) {
                    return position(c) * A2($elm$core$Basics$pow, alphaLen, partLen - i - 1);
                }), partList));
            });
            var _v0 = context;
            var guards = _v0.guards;
            ...
            var minHashLength = _v0.minHashLength;
            var alphabet = _v0.alphabet;
            var guardParts = A2($pdamoc$elm_hashids$Hashids$splitOn, guards, hash);
            var guardPartsLen = $elm$core$List$length(guardParts);
            var hash1 = A2($elm$core$Maybe$withDefault, '', $elm$core$List$head(2 <= guardPartsLen && guardPartsLen <= 3 ? A2($elm$core$List$drop, 1, guardParts) : guardParts));
            var hash2 = A2($elm$core$String$dropLeft, 1, hash1);
            var lotteryChar = A2($elm$core$String$left, 1, hash1);
            var go = F2(function(part, _v1) {
                var acc = _v1.a;
                var alpha = _v1.b;
                var alphaSalt = A2($elm$core$String$left, $elm$core$String$length(alpha), _Utils_ap(lotteryChar, _Utils_ap(salt, alpha)));
                var alpha1 = A2($pdamoc$elm_hashids$Hashids$reorder, alpha, alphaSalt);
                return _Utils_Tuple2(_Utils_ap(acc, _List_fromArray([
                    A2(unhash, part, alpha1)
                ])), alpha1);
            });
            var hashParts = A2($pdamoc$elm_hashids$Hashids$splitOn, seps, hash2);
            var numbers = $elm$core$String$isEmpty(hash1) ? _List_Nil : A3($elm$core$List$foldl, go, _Utils_Tuple2(_List_Nil, alphabet), hashParts).a;
            return !_Utils_eq(A2($pdamoc$elm_hashids$Hashids$encodeList, context, numbers), hash) ? _List_Nil : numbers;
        }
    });
    var $author$project$Main$wordFromRef = F2(function(words, wordRef) {
        return A2($elm$core$Maybe$andThen, function(_int) {
            return A2($elm_community$list_extra$List$Extra$getAt, _int, words);
        }, $elm$core$List$head(A2($pdamoc$elm_hashids$Hashids$decode, $author$project$Main$hashids, wordRef.id)));
    });
    var $author$project$Main$wordrefToUrl = function(_v0) {
        var lang = _v0.lang;
        var size = _v0.size;
        var id = _v0.id;
        return '#' + (lang + ('/' + ($elm$core$String$fromInt(size) + ('/' + id))));
    };
    var $author$project$Main$update = F2(function(msg, model) {
        var store = model.store;
        var wordRef = model.wordRef;
        var _v0 = _Utils_Tuple2(msg, model.state);
        _v0$10: while(true)switch(_v0.a.$){
            case 'BackSpace':
                if (_v0.b.$ === 'Ongoing') {
                    var _v1 = _v0.a;
                    var _v2 = _v0.b;
            ...
            case 'NewGame':
                var _v6 = _v0.a;
                return _Utils_Tuple2(A4($author$project$Main$initialModel, store, model.key, wordRef, $author$project$Game$Idle), A2($author$project$Main$startNewGame, store, $elm$core$Maybe$Nothing));
            case 'NewTime':
                var time = _v0.a.a;
                return _Utils_Tuple2(_Utils_update(model, {
                    time: time
                ...
                if (_v0.a.a.b.$ === 'Just') {
                    if (_v0.b.$ === 'Idle') {
                        ...
                        return _Utils_Tuple2(_Utils_update(model, {
                            ...
                            wordRef: _Utils_update(wordRef, {
                                id: hashid
                            })
                        }), $elm$core$Platform$Cmd$batch(_List_fromArray([
                            $author$project$Main$defocusMenuButtons,
                            A2($elm$browser$Browser$Navigation$pushUrl, model.key, $author$project$Main$wordrefToUrl(_Utils_update(wordRef, {
                                id: hashid
                            })))
                        ])));
                    } else break _v0$10;
            ...
            case 'Submit':
                if (_v0.b.$ === 'Ongoing') {
                    ...
                    var guesses = _v14.b;
                    var input = _v14.c;
                    var _v15 = A4($author$project$Game$validateGuess, store.lang, model.words, word, input);
                    if (_v15.$ === 'Ok') {
                        var guess = _v15.a;
            ...
            case 'SwitchLang':
                ...
                var newModel = A4($author$project$Main$initialModel, newStore, model.key, _Utils_update(wordRef, {
                    lang: $author$project$I18n$langToCode(lang)
                }), $author$project$Game$Idle);
            ...
            case 'UpdateWordSize':
                ...
                var newModel = A4($author$project$Main$initialModel, store, model.key, _Utils_update(wordRef, {
                    size: wordSize
                }), $author$project$Game$Idle);
                return _Utils_Tuple2(_Utils_update(newModel, {
                    wordSize: wordSize
            ...
            case 'UrlChanged':
                var url = _v0.a.a;
                var maybeWordRef = $author$project$Main$urlToWordref(url);
                if (_Utils_eq(maybeWordRef, $elm$core$Maybe$Just(wordRef))) return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
                else {
                    var newWordRef = A2($elm$core$Maybe$withDefault, wordRef, maybeWordRef);
                    var newStore = _Utils_update(store, {
                        lang: $author$project$I18n$parseLang(newWordRef.lang)
                    });
                    var newModel = A4($author$project$Main$initialModel, newStore, model.key, newWordRef, $author$project$Game$Idle);
                    return _Utils_Tuple2(newModel, A2($author$project$Main$startNewGame, newStore, maybeWordRef));
                }
            case 'WordsReceived':
                if (_v0.a.a.$ === 'Ok') {
            ...
            default:
                if (_v0.a.a.$ === 'Ok') {
                    var rawWords = _v0.a.a.a;
                    var words = A2($elm$core$List$filter, A2($elm$core$Basics$composeR, $elm$core$String$length, $elm$core$Basics$eq(model.wordSize)), A2($elm$core$List$filter, A2($elm$core$Basics$composeL, $elm$core$Basics$not, $elm$core$String$isEmpty), $elm$core$String$lines(rawWords)));
                    var maybeWordToFind = A2($author$project$Main$wordFromRef, words, model.wordRef);
                    var _v18 = function() {
                        if (maybeWordToFind.$ === 'Nothing') return _Utils_Tuple2($author$project$Game$Idle, A2($author$project$Main$getRandomWord, model.wordSize, words));
                        else {
                            var wordToFind = maybeWordToFind.a;
                            return _Utils_Tuple2(A3($author$project$Game$Ongoing, wordToFind, _List_Nil, ''), $elm$core$Platform$Cmd$none);
                        }
                    }();
                    var state = _v18.a;
                    var cmd = _v18.b;
    ...
    var $author$project$Main$view = function(model) {
        ...
        return {
            body: _List_fromArray([
                A2($author$project$Main$layout, model, function() {
                    switch(state.$){
                        case 'Idle':
                            return _List_fromArray([
