...
var $author$project$Main$Ongoing = {$: 1};
...
var $author$project$Main$randomWord = function (lang) {
	var words = $author$project$Main$getDictionnary(lang);
	return A2(
...
var $author$project$Main$init = F2(
	function (lang, _v0) {
		return _Utils_Tuple2(
			{r: _List_Nil, w: '', B: $elm$core$Maybe$Nothing, J: $author$project$Main$Ongoing, C: lang, I: $elm$core$Maybe$Nothing},
			A2(
				$elm$random$Random$generate,
				$author$project$Main$NewWord,
				$author$project$Main$randomWord(lang)));
	});
...
var $author$project$Main$try = F3(
	function (lang, word, input) {
		...
		return A2(
			...
			inputChars) ? $elm$core$Result$Err('Word must contains only alphabetic letters') : (($elm$core$String$length(input) !== 5) ? $elm$core$Result$Err('Word must be 5 letters long') : ((!A2(
			$elm$core$List$member,
			$elm$core$String$toLower(input),
			words)) ? $elm$core$Result$Err('Sorry, ' + (input + ' must be a word known from our dictionnary')) : $elm$core$Result$Ok(
			A3(
				$elm$core$List$map2,
				F2(
					function (a, b) {
						return _Utils_eq(a, b) ? $author$project$Main$Correct(a) : (A2($elm$core$List$member, a, wordChars) ? $author$project$Main$Misplaced(a) : $author$project$Main$Unused(a));
					}),
				inputChars,
				wordChars))));
	});
var $author$project$Main$update = F2(
	function (msg, model) {
		update:
		while (true) {
			var attempts = model.r;
			var currentTry = model.w;
			var language = model.C;
			switch (msg.$) {
				case 0:
					return A2($author$project$Main$init, language, 0);
				case 1:
					if (!msg.a.$) {
						var string = msg.a.a;
						return _Utils_Tuple2(
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									J: $author$project$Main$Errored('There was an issue while picking a word')
								}),
							$elm$core$Platform$Cmd$none);
					}
				case 3:
				...
				default:
					if (!msg.a.$) {
						var wordToFind = msg.a.a;
						var _v2 = A3($author$project$Main$try, language, wordToFind, currentTry);
						if (!_v2.$) {
							var attempt = _v2.a;
							return _Utils_Tuple2(
								_Utils_update(
									model,
									{
										r: A2($elm$core$List$cons, attempt, attempts),
										w: '',
...
var $author$project$Main$Lost = {$: 3};
...
var $author$project$Main$gameState = function (model) {
	return $author$project$Main$hasWon(model.r) ? $author$project$Main$Won : ((_Utils_cmp(
		$elm$core$List$length(model.r),
		$author$project$Main$maxAttempts) > -1) ? $author$project$Main$Lost : $author$project$Main$Ongoing);
};
var $elm$html$Html$h3 = _VirtualDom_node('h3');
var $elm$html$Html$input = _VirtualDom_node('input');
var $author$project$Main$SwitchLanguage = function (a) {
	return {$: 4, a: a};
};
...
var $author$project$Main$toString = function (language) {
	if (!language) {
		return 'english';
	} else {
		return 'french';
...
var $author$project$Main$languageButton = F2(
	function (lang, isActive) {
		var classes = isActive ? 'btn btn-secondary active' : 'btn btn-secondary';
		return A2(
			$elm$html$Html$label,
			_List_fromArray(
				[
				...
				[
					...
					_List_fromArray(
						[
							...
							$author$project$Main$SwitchLanguage(lang)),
							$elm$html$Html$Attributes$checked(isActive)
						]),
					_List_Nil),
					$elm$html$Html$text(' '),
					$elm$html$Html$text(
					$author$project$Main$toString(lang))
				]));
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2($author$project$Main$languageButton, 0, !language),
						A2($author$project$Main$languageButton, 1, language === 1)
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						$elm$html$Html$text(
						$author$project$Main$toString(language)),
				...
				function () {
				var _v0 = $author$project$Main$gameState(model);
				switch (_v0.$) {
					case 0:
						var gameError = _v0.a;
						return A2(
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$elm$html$Html$text('The game encountered an error')
										])),
						...
						return A2(
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$elm$html$Html$text('Bummer!')
										])),
									function () {
									if (!word.$) {
										var wordToFind = word.a;
										return A2(
											$elm$html$Html$p,
											_List_Nil,
											_List_fromArray(
												[
													$elm$html$Html$text('The word to find was '),
													A2(
													$elm$html$Html$strong,
													_List_Nil,
													_List_fromArray(
														[
															$elm$html$Html$text(wordToFind)
														])),
													$elm$html$Html$text('.')
												]));
									} else {
								...
								[
									...
									_List_fromArray(
										[
											...
											$author$project$Main$Submit(word))
										]),
									_List_fromArray(
										[
											A2(
											...
											_List_fromArray(
												[
													...
													$elm$html$Html$Attributes$value(currentTry)
												]),
											_List_Nil),
											A2(
											$elm$html$Html$button,
									...
									_List_fromArray(
										[
											$elm$html$Html$text('Enter a 5 letters '),
											$elm$html$Html$text(
											$author$project$Main$toString(language)),
